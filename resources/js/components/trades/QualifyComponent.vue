...
<script>
  export default {
    data() {
      ...
      mounted() {
        axios.get(localStorage['URLroot'] + '/GetHostQualify').then(response => (this.GetQualify(response.data)));
        axios.get(localStorage['URLroot'] + '/MyComments').then(response => (this.MyComments=response.data));
      },        
  }
</script>